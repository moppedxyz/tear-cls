...
from tearcls.augment import AUG_NAMES, build_train_augment, crop_afm_data
...
FIELDNAMES = [
    "filepath", "class", "label", "patient_code",
    "split", "source_filepath", "variant",
]
...
def main() -> None:
    ...
    ap.add_argument("--n", type=int, default=10, help="Augmented variants per train image")
    ap.add_argument("--seed", type=int, default=3407, help="Seed for reproducible dumps")
    ap.add_argument(
        "--enable", type=str, default=None,
        help=f"Comma-separated aug names (known: {','.join(AUG_NAMES)})",
    )
    ap.add_argument("--disable", type=str, default=None, help="Comma-separated aug names to skip")
    ...
    with index_path.open("w", newline="") as f_out:
        ...
        for row in rows:
            ...
            writer.writerow({
                "filepath": orig_path.relative_to(REPO_ROOT).as_posix(),
                "class": row["class"],
                "label": row["label"],
            ...
            for i in range(args.n):
                ...
                writer.writerow({
                    "filepath": aug_path.relative_to(REPO_ROOT).as_posix(),
                    "class": row["class"],
                    "label": row["label"],
                    "patient_code": row["patient_code"],
                    "split": row["split"],
